...
const CampaignForm = ({ theCampaign, formId, campaignId, newCampaign = true }) => {
  const { name, from_date, to_date, total_budget, daily_budget, creative_upload } =
    theCampaign;
  ...
  const submitFormToMakeRequest = async (values) => {
    ...
    if (!campaignImage && !campaignImageURL)
      return toast.warning("You need to add an image to continue!");
    ...
    <div>
      <div className="campaign_form_bg">
        <div className="campaign_form_img_bg ">
          ...
          {!campaignImageURL && <h1> Click to add an image!</h1>}
        </div>
        <Formik
          validationSchema={validation}
          onSubmit={submitFormToMakeRequest}
          ...
          {({ values }) => (
            <Form className="campaign_form_form_bg" id={formId}>
              ...
              <div className="campaign_form_button_bg">
                ...
                <button
                  type="submit"
                  disabled={createCampaignIsLoading}
                  className="campaign_form_submit_button"
                >
